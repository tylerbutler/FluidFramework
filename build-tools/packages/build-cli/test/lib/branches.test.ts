...
import { ReleaseGroup } from "../../src/releaseGroups";
...
describe("generateBumpDepsBranchName", () => {
    it("semver versions", () => {
        it("patch", () => {
            const actual = generateBumpDepsBranchName(ReleaseGroup.Azure, "patch");
            const expected = "bump_deps_azure_patch";
            assert.equal(actual, expected);
        });

        it("minor", () => {
            const actual = generateBumpDepsBranchName(ReleaseGroup.Azure, "minor");
            const expected = "bump_deps_azure_minor";
            assert.equal(actual, expected);
        });

        it("major", () => {
            const actual = generateBumpDepsBranchName(ReleaseGroup.Azure, "major");
...
describe("generateReleaseBranchName", () => {
    it("semver", () => {
        const actual = generateReleaseBranchName(ReleaseGroup.Azure, "1.2.3");
        const expected = "release/azure/1.2";
        assert.equal(actual, expected);
    });

    it("virtualPatch version scheme", () => {
        const actual = generateReleaseBranchName(ReleaseGroup.BuildTools, "0.4.2000");
        const expected = "release/build-tools/0.4.2000";
        assert.equal(actual, expected);
    });

    it("virtualPatch patch", () => {
        const actual = generateReleaseBranchName(ReleaseGroup.BuildTools, "0.4.2002");
        const expected = "release/build-tools/0.4.2000";
        assert.equal(actual, expected);
    });

    it("client using semver", () => {
        const actual = generateReleaseBranchName(ReleaseGroup.Client, "1.2.3");
        const expected = "release/1.2";
        assert.equal(actual, expected);
    });

    it("Fluid internal version scheme", () => {
        const actual = generateReleaseBranchName(ReleaseGroup.Client, "2.0.0-internal.1.0.0");
