...
import { knownReleaseGroupsEnum } from "../../src/releaseGroups";
...
describe("generateReleaseBranchName", () => {
	...
	it("virtualPatch version scheme", () => {
		const actual = generateReleaseBranchName(knownReleaseGroupsEnum["build-tools"], "0.4.2000");
		const expected = "release/build-tools/0.4.2000";
		assert.equal(actual, expected);
	});

	it("virtualPatch patch", () => {
		const actual = generateReleaseBranchName(knownReleaseGroupsEnum["build-tools"], "0.4.2002");
		const expected = "release/build-tools/0.4.2000";
		assert.equal(actual, expected);
	});

	it("client using semver", () => {
		const actual = generateReleaseBranchName(knownReleaseGroupsEnum.client, "1.2.3");
