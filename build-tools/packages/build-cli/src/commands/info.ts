...
const nameToColumnInfo: Record<string, ColumnInfo<unknown>> = {
	releaseGroup: {
		getValue: (pkg: IPackage) => pkg.releaseGroup ?? "n/a",
		style: { alignment: "left" },
	},
	name: { getValue: (pkg: IPackage) => pkg.name, style: { alignment: "left" } },
	private: {
		getValue: (pkg: IPackage) => pkg.packageJson.private === true,
		formatValue: (value) => (value === true ? "-private-" : ""),
		style: { alignment: "center" },
	},
	version: {
		getValue: (pkg: IPackage) => pkg.version,
		style: { alignment: "left" },
	},
	path: {
		getValue: (pkg: IPackage) => pkg.directory,
		style: { alignment: "left" },
	},
};
	...
	async run(): Promise<Record<string, unknown>[]> {
		...
		let packages =
			flags.releaseGroup === undefined
				...
				: (repo.releaseGroups.get(flags.releaseGroup)?.packages ?? []);
