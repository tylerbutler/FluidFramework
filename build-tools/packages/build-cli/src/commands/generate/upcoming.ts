...
> {
	...
	public async run(): Promise<string> {
		...
		const changesetDir = path.join(releaseGroup.packages[0].directory, DEFAULT_CHANGESET_PATH);
		const changes = await loadChangesets(changesetDir, logger);

		const { version } = releaseGroup.packages[0];
		...
		const outputPath = path.join(context.repo.root, flags.out);
