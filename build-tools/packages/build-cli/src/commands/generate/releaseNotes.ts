...
> {
	...
	public async run(): Promise<string> {
		...
		// Create mdast nodes for document structure
		const createHeaderComment = (): Html => ({
			type: "html",
			value: "<!-- THIS IS AN AUTOGENERATED FILE. DO NOT EDIT THIS FILE DIRECTLY. -->",
		});

		const createTitle = (): Heading => ({
			type: "heading",
			depth: 1,
			children: [
				{
					type: "text",
					value: `Fluid Framework v${version}`,
				},
			],
		});

		const createContentsHeading = (): Heading => ({
			type: "heading",
			depth: flags.excludeH1 ? 2 : 2,
			children: [
				{
					type: "text",
					value: "Contents",
		...
		const createFooter = (): Root => {
			const footerMarkdown = `### 🛠️ Start Building Today!
...
[Issue](https://github.com/microsoft/FluidFramework/issues) pages as you adopt Fluid Framework!`;
			return fromMarkdown(footerMarkdown);
		};
		...
		// Create the main document structure as mdast nodes
		const documentChildren: Root["children"] = [];

		// Add header comment
		documentChildren.push(createHeaderComment());

		// Add title if not excluded
		if (!flags.excludeH1) {
			documentChildren.push(createTitle());
		}

		// Add contents heading
		documentChildren.push(createContentsHeading());

		// Build sections as mdast nodes
		for (const [name, { heading: sectionHead }] of sectionsToBuild.entries()) {
			...
			documentChildren.push(sectionHeading);
			...
			for (const change of changes) {
				if (
					...
					// Create change title heading
					const changeTitleHeading: Heading = {
						type: "heading",
						depth: 3,
						children: [
							{
					...
					documentChildren.push(changeTitleHeading);

					// Add change body as paragraph(s)
					if (change.body.trim()) {
						const changeBodyNodes = fromMarkdown(change.body);
						documentChildren.push(...changeBodyNodes.children);
					}

					// Create "Change details" heading
					const changeDetailsHeading: Heading = {
						type: "heading",
						depth: 4,
						children: [
							{
					...
					documentChildren.push(changeDetailsHeading);

					// Add commit info if available
					if (change.commit?.sha !== undefined) {
						const commitParagraph: Paragraph = {
							type: "paragraph",
							children: [
								{
									type: "text",
									value: `Commit: ${change.commit.sha}`,
								},
							],
						};
						documentChildren.push(commitParagraph);
					}

					// Add affected packages
					const affectedPackagesList = {
						type: "paragraph" as const,
						children: [
							{
								type: "text" as const,
								value: "Affected packages:",
							},
						],
					};
					documentChildren.push(affectedPackagesList);

					const packageListMarkdown = Object.keys(change.metadata)
						.map((pkg) => `- ${pkg}`)
						.join("\n");

					if (packageListMarkdown) {
						const packageListNodes = fromMarkdown(packageListMarkdown);
						documentChildren.push(...packageListNodes.children);
					}

					// Add TOC link
					const tocLink: Link = {
						type: "link",
						url: `#${flags.headingLinks ? "user-content-" : ""}contents`,
						children: [
							{
								type: "text",
								value: RELEASE_NOTES_TOC_LINK_TEXT,
							},
						],
					};
					...
					documentChildren.push(tocParagraph);
				} else {
					this.info(
		...
		documentChildren.push(...footerNodes.children);

		// Create the complete document as an mdast Root node
		const document: Root = {
			type: "root",
		...
		// Convert the mdast document to markdown string, then process it
		const markdownString = toMarkdown(document);
		const contents = String(await processor.process(markdownString));
