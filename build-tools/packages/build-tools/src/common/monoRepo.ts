...
export class Workspace {
	...
	static load(name: string, workspaceDefinition: WorkspaceDefinition): Workspace {
		const { directory, defaultInterdependencyRange } = workspaceDefinition;
		...
		const packageDirs = packages.filter((pkg) => pkg.relativeDir !== ".").map((pkg) => pkg.dir);
	...
	constructor(
		...
		if (existsSync(lernaPath)) {
			...
			if (lerna.version !== undefined) {
				traceInit(
					`${name} (workspace): Loading version (${lerna.version}) from ${lernaPath}`,
				);
