...
export default class BuildCommand extends CommandWithConfig<typeof BuildCommand, SailConfig> {
	...
	static override readonly flags = {
		...
		task: Flags.string({
			char: "t",
			description: "Task to execute. Can be specified multiple times to run multiple tasks.",
			default: ["build"],
			multiple: true,
		}),
	...
	static override readonly examples: Command.Example[] = [
		...
		{
			description: "Run the 'clean' task followed by the 'build' task on the current project.",
	...
	protected async build(timer: Timer): Promise<void> {
		...
		const timeInMinutes =
			timer.getTotalTime() > 60000
				...
				: "";
		this.verbose(`Total time: ${(timer.getTotalTime() / 1000).toFixed(3)}s${timeInMinutes}`);
	...
	private getBuildOptions(): FluidBuildOptions {
		...
		const newOptions: FluidBuildOptions = {
			...
			workerThreads: this.commandConfig?.workerThreads ?? this.defaultConfig.workerThreads,
