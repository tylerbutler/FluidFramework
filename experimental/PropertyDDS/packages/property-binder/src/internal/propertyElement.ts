...
function hasGetValues(property: Property): boolean {
	return (
		(property as any).getValues &&
		(property!.getTypeid() !== "String" || isCollection(property))
	);
}
...
export class PropertyElement {
	...
	isReferenceContainer(): boolean {
		const reference = this.isValid()
			? TypeIdHelper.isReferenceTypeId(this._property!.getTypeid())
			: false;
	...
	isPrimitiveCollection(): boolean {
		if (
			this._property &&
			(this._property.getContext() === "array" || this._property.getContext() === "map")
		) {
	...
	getChildIds(): string[] | undefined {
		if (!this.isValid() || this._childToken !== undefined || !(this._property as any).getIds) {
			//@TODO find a common type which has getIds
			return undefined;
		} else {
	 ...
	 */
	getChild(
		in_child: string[] | string,
		in_options = Options,
		in_tokensTypes?: PathHelper.TOKEN_TYPES[],
	): PropertyElement {
	...
	becomeChild(
		in_child: string[] | string,
		in_options = Options,
		in_tokensTypes: Array<PathHelper.TOKEN_TYPES> = [],
	): PropertyElement {
		...
		const isAlways =
			...
			resolutionMode === BaseProperty.REFERENCE_RESOLUTION.ALWAYS;
		...
		for (let i = 0; i < child.length && this.isValid(); ++i) {
			...
			if (
				i === 0 &&
				in_tokensTypes[i] === PathHelper.TOKEN_TYPES.PATH_ROOT_TOKEN &&
				token === "/"
			) {
				this._property = this._property!.getRoot();
				this._childToken = undefined;
			} else if (
				in_tokensTypes[i] === PathHelper.TOKEN_TYPES.DEREFERENCE_TOKEN &&
				token === "*"
			) {
				...
				in_tokensTypes[i] === PathHelper.TOKEN_TYPES.RAISE_LEVEL_TOKEN &&
				token === "../"
			) {
				this.becomeParent();
			} else {
