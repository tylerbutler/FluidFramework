...
PluginManager.instance = {
	...
	instanceMonitor: new InstanceMonitor({
		port: settings.get("mh:internal:port"),
		shortServiceName: "MH",
	}),
...
let allowedHeaders =
	"user-id, client-id, x-ads-token-data, Origin, X-Requested-With, Content-Type, Accept";
let runningOnCloudOS =
	process.env.hasOwnProperty("APP_MONIKER") && process.env.hasOwnProperty("CLOUDOS_MONIKER");
if (!runningOnCloudOS) {
...
let promise = new Promise((resolve, reject) => {
	...
	let materializedHistoryServer = new Server({
	...
	materializedHistoryServer
		.start()
		.then(() => resolve(materializedHistoryServer))
		.catch(reject);
});
