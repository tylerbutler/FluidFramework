...
class RepairManager {
	...
	async scanAndRepairBranches(params) {
		const branchProcessingQueue = asyncQueue(
			this._makeBranchConsistent.bind(this),
			params.dop || 1,
		);
		let filteredResult = {};
		...
		do {
			...
			for (const branchGuid of filteredResult.branches) {
				...
				if (branchInfo.branch.parent) {
					...
					pushedBackBranches.push({
						guid: branchGuid,
						parent: branchInfo.branch.parent.branch.guid,
					});
				} else {
					branchProcessingQueue.push({
						branchGuid,
						since: params.lastModifiedSince,
					});
				}
			}
	...
	async _makeBranchConsistent({ branchGuid, since }) {
		...
		const mhEnabled =
			branch.meta &&
			branch.meta.materializedHistory &&
			branch.meta.materializedHistory.enabled;
		if (!mhEnabled) {
		...
		try {
			...
			if (err instanceof OperationError && err.statusCode === HTTPStatus.NOT_FOUND) {
				logger.trace(
					`Branch '${branchGuid}' does not have Materialized History. Creating it from scratch.`,
				);
				...
				await this._forceRepairBranch(
					{ guid: branch.guid, headCommitGuid: rootCommitGuid },
					branch.head.guid,
				);
		...
		if (localBranch.headCommitGuid !== branch.head.guid) {
			...
			try {
				...
				if (err instanceof OperationError && err.statusCode === HTTPStatus.NOT_FOUND) {
					logger.trace(
						`MH is ahead for branch '${branchGuid}'. Deleting commits until head is at ` +
							branch.head.guid,
					);
		...
		logger.trace(
			`Last consistent commit for branch '${branchGuid}' is '${localBranch.headCommitGuid}'`,
		);
		...
		while (headCommitGuid !== targetCommitGuid) {
			...
			try {
				...
				if (err instanceof OperationError && err.statusCode === HTTPStatus.NOT_FOUND) {
					...
					logger.trace(
						`Cannot delete commits beyond '${headCommitGuid}' due to inconsistencies in branch ` +
							branch.guid,
					);
	...
	async _getLatestConsistentCommit(branch, since) {
		...
		while (
			!consistentCommitGuid ||
			(consecutiveConsistentCommits < CONSISTENT_COMMIT_LIMIT && commitDate >= since)
		) {
			...
			if (headCommitGuid === branch.rootCommitGuid) {
				...
				try {
					await this._mhService.getCommitMV({
						guid: headCommitGuid,
						fetchSchemas: true,
						branchGuid: branch.guid,
					});
					...
					if (err instanceof OperationError && err.statusCode === HTTPStatus.NOT_FOUND) {
						...
						logger.trace(
							`Root commit '${headCommitGuid}' is inconsistent for branch '${branch.guid}'. Re-creating.`,
						);
						try {
							await this._hfdmClassicClient.getCommit({
								branchGuid: branch.guid,
							...
							if (
								err2 instanceof OperationError &&
								err2.statusCode === HTTPStatus.NOT_FOUND
							) {
								// We delete the commit node, so that the branch creation does not think we are branching
								// from an existing commit.
								await this._mhService._storageManager.delete(
									"commit:" + branch.rootCommitGuid,
								);
							} else {
			...
			try {
				...
				if (err instanceof OperationError && err.statusCode === HTTPStatus.NOT_FOUND) {
					...
					logger.trace(
						`Commit '${headCommitGuid}' node is missing for branch '${branch.guid}'`,
					);
			...
			try {
				...
				if (err instanceof OperationError && err.statusCode === HTTPStatus.NOT_FOUND) {
					logger.trace(
						`Commit '${headCommitGuid}' has an inconsistent tree for branch '${branch.guid}'`,
					);
					// We want to redo this commit, so our consistent commit needs to be before this one
					consistentCommitGuid = undefined;
					consecutiveConsistentCommits = 0;
