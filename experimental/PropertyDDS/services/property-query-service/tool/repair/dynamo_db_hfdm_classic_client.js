...
class DynamoDBHfdmClassicClient {
	...
	constructor() {
			...
			instance = {
				systemMonitor: pm
					...
					.createInstance(
						...
						ModuleLogger.getLogger("HFDM.MaterializedHistoryRepair.SystemMonitor"),
						"mr",
		...
		const dependencies = {
			...
			binaryStore: new BinaryStore({
				settings: ddbSettings.get("binary"),
			}),
		};
	...
	async getBranch(branchGuid) {
		...
		const repositoriesResult = await this._propertyGraph.getRepositories([
			branchesResult.branches[0].container.guid,
		]);
	...
	async scanBranches(params) {
		...
		if (scanResult.LastEvaluatedKey) {
			result.lastEvaluatedKey = AWS.DynamoDB.Converter.unmarshall(
