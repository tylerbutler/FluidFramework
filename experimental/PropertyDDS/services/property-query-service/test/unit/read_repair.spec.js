...
describe("Read repair", () => {
	...
	before(() => {
		...
		sandbox.stub(mhService._branchManager, "getBranch").withArgs(branchGuid).resolves({
			guid: branchGuid,
			headCommitGuid,
		});
		getCommitRangeStub = sandbox.stub(pssClient, "getCommitRange").resolves({
			commits: [
				{
		...
		createCommitStub = sandbox.stub(mhService._commitManager, "createCommit").resolves({});
		sandbox
			.stub(storage, "get")
			.withArgs("commit:" + nonExistingCommitGuid)
			.resolves(undefined)
			.withArgs("commit:" + existingCommitGuid)
			.resolves({
