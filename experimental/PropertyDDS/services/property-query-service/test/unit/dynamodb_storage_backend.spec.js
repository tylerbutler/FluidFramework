...
describe("DynamoDB Storage Backend", () => {
	...
	describe("Reading nodes", () => {
		describe("Stored inline", () => {
			...
			describe("With a dynamodb failure", () => {
				before(() => {
					batchGetItemStub = sandbox
						.stub(DynamoClientAPI.ddbClient, "batchGetItem")
						.rejects(new Error("Something went super wrong with DynamoDB"));
				});

				it("should bubble the error up", () =>
					expect(ddbStorage.get(theKey)).to.be.rejectedWith(
						"Something went super wrong with DynamoDB",
					));
			...
			describe("With success and an existing value", () => {
				...
				before(() => {
					sandbox.stub(DynamoClientAPI.ddbClient, "batchGetItem").resolves({
				...
				it("should return the value", () =>
					expect(ddbStorage.get(theKey)).to.eventually.eql(theValue));
			...
			describe("With success and no value", () => {
				...
				it("should return the value", () =>
					expect(ddbStorage.get(theKey)).to.eventually.eql(undefined));
		...
		describe("Stored in S3", () => {
			...
			describe("with a S3 failure", () => {
				before(() => {
					getObjectStub = sandbox
						.stub(BigStoreAPI, "getObject")
						.withArgs(theS3Key)
				...
				it("should bubble the error up", () =>
					expect(ddbStorage.get(theKey)).to.be.rejectedWith(
						"S3 didn't feel like cooperating today.  Sorry.",
					));

				it("should have called S3", () => expect(getObjectStub.called).to.eql(true));
			});
	...
	describe("Without a write batch started", () => {
		...
		describe("calling finishWriteBatch", () => {
			it("should throw", () =>
				expect(() => ddbStorage.finishWriteBatch(fakeBatchId)).to.throw(
					"Trying to finish a non-started batch",
				));
		});
	});
	...
	describe("With a started write batch", () => {
		...
		describe("with store() called twice with small payloads", () => {
			...
			describe("calling finishWriteBatch", () => {
				...
				describe("with a DynamoDB failure", () => {
					before(() => {
						batchWriteItemStub = sandbox
							.stub(DynamoClientAPI.ddbClient, "batchWriteItem")
							.withArgs(expectedArgs)
							.rejects(new Error("Something went super wrong with DynamoDB"));
					});

					it("should reject with the error", () =>
						expect(ddbStorage.finishWriteBatch(createdBatch)).to.be.rejectedWith(
							"Something went super wrong with DynamoDB",
						));
				...
				describe("with success in DynamoDB", () => {
					...
					it("should resolve", () =>
						expect(ddbStorage.finishWriteBatch(createdBatch)).to.be.eventually
							.fulfilled);
		...
		describe("with store called twice with large payloads", () => {
			...
			let item1Value = JSON.stringify({
				a: Array(LARGE_STRING_SIZE).fill("a").join(""),
			});
			let item2Value = JSON.stringify({
				b: Array(LARGE_STRING_SIZE).fill("b").join(""),
			});
			...
			describe("calling finishWriteBatch", () => {
				describe("with an S3 failure", () => {
					before(() => {
						putObjectStub = sandbox
							.stub(BigStoreAPI, "putObject")
							.rejects(new Error("S3 API did not cooperate today.  Sorry"));
					});

					before(() => {
						deleteAllStub = sandbox
							.stub(BigStoreAPI, "deleteAll")
							.rejects(
								new Error("S3 API for delete did not cooperate today.  Sorry"),
							);
					});
					...
					it("should bubble up the exception", () =>
						expect(ddbStorage.finishWriteBatch(createdBatch)).to.be.rejectedWith(
							"S3 API did not cooperate today.  Sorry",
						));
				...
				describe("with an S3 success", () => {
					before(() => {
						putObjectStub = sandbox.stub(BigStoreAPI, "putObject").resolves();
					});
					...
					describe("with a DynamoDB failure", () => {
						...
						before(() => {
							deleteAllStub = sandbox.stub(BigStoreAPI, "deleteAll").resolves();
						});

						it("should bubble up the error", () =>
							expect(ddbStorage.finishWriteBatch(createdBatch)).to.be.rejectedWith(
								"DynamoDB failed... Oops.",
							));
					...
					describe("with a DynamoDB success", () => {
						...
						it("should resolve", () =>
							expect(ddbStorage.finishWriteBatch(createdBatch)).to.be.fulfilled);
		...
		describe("with update() called with a large payload and a previously large payload", () => {
			...
			beforeEach(() => {
				...
				ddbStorage.update(createdBatch, item2Id, item2Value, {
					originalNodeSize: 600 * 1000,
				});
			});
			...
			describe("calling finishWriteBatch", () => {
				...
				describe("calling finishWriteBatch twice", () => {
					...
					it("should resolve the first one and throw on the second one", () => {
						...
						return Promise.all([
							expect(ddbStorage.finishWriteBatch(createdBatch)).to.be.fulfilled,
							expect(() => ddbStorage.finishWriteBatch(createdBatch)).to.throw(
								"Trying to finish an already finished batch",
							),
						]);
					});
				});
			...
			describe("calling finishWriteBatch concurrently with store", () => {
				...
				it("should throw on the store", () => {
					return Promise.all([
						...
						expect(() => ddbStorage.store(aBatch, "some-id", { a: "value" })).to.throw(
							"Trying to store in an already finished batch",
						),
					]);
				});
			});
			...
			describe("calling finishWriteBatch concurrently with update", () => {
				...
				it("should throw on the update", () => {
					return Promise.all([
						ddbStorage.finishWriteBatch(aBatch),
						expect(() =>
							ddbStorage.update(aBatch, "anothyer-id", {
								a: "value",
							}),
	...
	describe("deleting nodes", () => {
		...
		describe("without an S3 reference", () => {
			describe("successfully", () => {
				...
				it("should do the deletion", () => ddbStorage.delete(theKey));
