...
} = require("@fluid-experimental/property-query");
...
describe("Branch deletion integration test", function () {
			...
			describe("with two commits", () => {
				...
				describe("with an invalid request signature", () => {
					it("should reject the creation request", () =>
						expect(Fixtures.deleteBranches([createdBranchGuid], {})).to.be.rejectedWith(
							Error,
				...
				describe("Branch deletion", () => {
					...
					it("should poll the task until completion", async () => {
						do {
							...
							expect(taskInfo.status).not.to.eql(
								"FAILED",
								"task was failed: " + taskInfo.error,
							);
						} while (taskInfo.status !== "COMPLETED");
					});

					it("should reject the poll without a signature", () =>
						expect(Fixtures.fetchDeleteTask(taskInfo.taskUrl, {})).to.be.rejectedWith(
							Error,
							"Request signature algorithm not supported",
						));
					...
					it("should return a 404 when trying to fetch the branch", function () {
						let headers = Fixtures.getRequestSignatureHeaders(createdBranchGuid);
						return expect(
							Fixtures.fetchBranch(createdBranchGuid, headers),
	...
	describe("For a branch not existing", () => {
		...
		describe("Branch deletion", () => {
			...
			it("should return a 404 when trying to fetch the branch", function () {
				let headers = Fixtures.getRequestSignatureHeaders(branchGuid);
				return expect(Fixtures.fetchBranch(branchGuid, headers)).to.be.rejectedWith(
					Error,
					"Not Found",
				);
			});
		...
		describe("On a properly created branch", () => {
			...
			describe("with two commits", () => {
				...
				describe("Branch deletion", () => {
					...
					it("should call the branch deletion", async () => {
						let headers =
							Fixtures.getRequestSignatureHeaders(conflictCreatedBranchGuid);
						existingTasks = await Fixtures.deleteBranches(
							[conflictCreatedBranchGuid],
							headers,
						);
					});
					...
					it("should restart the task", async () => {
						let headers =
							Fixtures.getRequestSignatureHeaders(conflictCreatedBranchGuid);
						taskInfo = await Fixtures.retryBranchDeletion(
							existingTasks.existingTasks[0].taskGuid,
							headers,
						);
					});
					...
					it("should return a 404 when trying to fetch the branch", function () {
						let headers =
							Fixtures.getRequestSignatureHeaders(conflictCreatedBranchGuid);
						return expect(
							Fixtures.fetchBranch(conflictCreatedBranchGuid, headers),
						).to.be.rejectedWith(Error, "Not Found");
					});
				});
			});
			...
			describe("with two commits", () => {
				...
				describe("Branch deletion retry", () => {
					...
					it("should reject the retry without a signature", () =>
						expect(
							Fixtures.retryBranchDeletion(theFailedTask.taskGuid, {}),
					...
					it("should poll the task until completion", async () => {
						do {
							let headers =
								Fixtures.getRequestSignatureHeaders(secondCreatedBranchGuid);
							taskInfo = await Fixtures.fetchDeleteTask(taskInfo.taskUrl, headers);
							expect(taskInfo.status).not.to.eql(
								"FAILED",
								"task was failed: " + taskInfo.error,
					...
					it("should return a 404 when trying to fetch the branch", function () {
						let headers = Fixtures.getRequestSignatureHeaders(secondCreatedBranchGuid);
						return expect(
							Fixtures.fetchBranch(secondCreatedBranchGuid, headers),
						).to.be.rejectedWith(Error, "Not Found");
					});
				});
			});
			...
			describe("with two commits", () => {
				...
				describe("Branch deletion retry", () => {
					...
					it("should have some of the nodes already deleted", async () => {
						let storageManager = server._materializedHistoryService._storageManager;
						let deletePromises = theFailedTask.nodesToDelete
							.slice(0, 10)
							.map((nodeRef) => storageManager.delete(nodeRef));
					...
					it("should poll the task until completion", async () => {
						do {
							let headers =
								Fixtures.getRequestSignatureHeaders(thirdCreatedBranchGuid);
							taskInfo = await Fixtures.fetchDeleteTask(taskInfo.taskUrl, headers);
							expect(taskInfo.status).not.to.eql(
								"FAILED",
								"task was failed: " + taskInfo.error,
					...
					it("should return a 404 when trying to fetch the branch", function () {
						let headers = Fixtures.getRequestSignatureHeaders(thirdCreatedBranchGuid);
						return expect(
							Fixtures.fetchBranch(thirdCreatedBranchGuid, headers),
						).to.be.rejectedWith(Error, "Not Found");
					});
	...
	describe("with a missing task", () => {
		it("should reject the poll of a non-existing task", function () {
			let headers = Fixtures.getRequestSignatureHeaders(thirdCreatedBranchGuid);
			return expect(
				Fixtures.fetchDeleteTask(
					`http://127.0.0.1:${port}/v1/branchDeletion/anyweirdtask`,
					headers,
				),
			).to.be.rejectedWith(Error, "Not found");
		});
		...
		it("should reject the retrial of a non-existing task", function () {
			let headers = Fixtures.getRequestSignatureHeaders(thirdCreatedBranchGuid);
			return expect(
				Fixtures.retryBranchDeletion("wrongTaskGuid", headers),
			).to.be.rejectedWith(Error, "Not found");
		});
	});
