...
describe("Branch consistency check", function () {
	...
	const compareTreeNodeRefSets = (expected, actual) => {
		compareRefSets(
			expected.filter((ref) => /^(l:|i:)/.test(ref)),
	...
	const setup = async () => {
		...
		sandbox
			.stub(hfdmClassicClient, "getCommitRange")
			.callsFake(({ branchGuid: guid, minCommitGuid: min, maxCommitGuid: max, limit: l }) => {
				...
				return Promise.resolve({
					commits: commits.slice(minIndex + 1, Math.min(l, maxIndex - minIndex + 1)),
				});
			});
		sandbox.stub(hfdmClassicClient, "getCommit").callsFake(({ branchGuid: bg, commitGuid }) => {
		...
		sandbox.stub(hfdmClassicClient, "getBranch").withArgs(branchGuid).resolves(branchInfo);
	...
	describe("when branch does not exist", () => {
		...
		before(async () => {
			...
			updatedRefs = [];
			deletedRefs = [];
			await repairManager._makeBranchConsistent({
				branchGuid,
				since: startDate,
			});
		});
