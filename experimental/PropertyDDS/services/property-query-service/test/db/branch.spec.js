...
describe("Branch creation integration test", function () {
	...
	it("should fail to fetch that branch with an invalid request signature", () => {
		return expect(Fixtures.fetchBranch(createdBranchGuid, {})).to.rejectedWith(
			Error,
	...
	describe("when branching from an existing commit", () => {
		...
		it("should be possible to commit to different branches and get correct MVs", async () => {
			...
			const fetchAtA = await Fixtures.fetchMaterializedView(
				createdBranchGuid,
				commitGuidA,
				headersA,
				{},
			);
			const fetchAtB = await Fixtures.fetchMaterializedView(
				branchedBranchGuid,
				commitGuidB,
				headersB,
				{},
