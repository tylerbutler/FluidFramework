...
describe("Index db tests", () => {
	...
	describe("when creating simple one column indices through branch metadata", () => {
		...
		before(() => {
			...
			indexNames = [
				...
				"thingsByAge",
				"peopleByHavingFun",
				"peopleByDistanceToUranus",
			];
			indexDefs = [
				{
					fields: [{ typeId: "String", name: "firstName" }],
					include: [{ schema: "example:person-1.0.0" }],
				},
				{
					fields: [{ typeId: "Integer", name: "age" }],
					include: [
				...
				{
					fields: [{ typeId: "Boolean", name: "havingFun" }],
					include: [{ schema: "example:person-1.0.0" }],
				},
				{
					fields: [{ typeId: "Double", name: "distanceToUranus" }],
					include: [{ schema: "example:person-1.0.0" }],
				},
			];
			const meta = {
		...
		it("should have created a root commit node for each index", async () => {
			for (let i = 0; i < indexNames.length; i++) {
				const indexCommitRootNode = await backend.get(
					`commitIndex#${branchGuid}#${indexNames[i]}:${rootCommitGuid}`,
				);
		...
		describe("when inserting data that matches the index include criteria", () => {
			...
			it("should produce a new commit node for each index", async () => {
				for (let i = 0; i < indexNames.length; i++) {
					const firstCommit = await backend.get(
						`commitIndex#${branchGuid}#${indexNames[i]}:${firstCommitGuid}`,
					);
			...
			it("should produce the expected MV for each index", async () => {
				const mvExpectations = [
					...
					createSingleColumnIndexMV([
						...
						[
							new Long(0xffffffff, 0xffffffff).toString(),
							"extraterrestrials[00000000-0000-0000-0000-000000000011]",
						],
						[
							new Long(0xffffffff, 0xffffffff, true).toString(),
							"planets[00000000-0000-0000-0000-000000000021]",
						],
					]),
					createSingleColumnIndexMV([
		...
		describe("when modifying data that matches the index include criteria", () => {
			...
			it("should produce a new commit node for each index", async () => {
				for (let i = 0; i < indexNames.length; i++) {
					const secondCommit = await backend.get(
						`commitIndex#${branchGuid}#${indexNames[i]}:${secondCommitGuid}`,
					);
	...
	describe("when validating index definition", () => {
		...
		it("should require a field", () => {
			...
			return expect(
				mhService.createBranch({
					guid: branchGuid,
					rootCommitGuid,
					meta,
				}),
			)
				.to.be.eventually.rejectedWith('"fields" must contain at least 1 items')
		...
		it("should require name for a field", () => {
			...
			return expect(
				...
				.to.be.eventually.rejectedWith('"name" is not allowed to be empty')
				.and.to.have.property("statusCode", 400);
		});
		...
		it("should validate the name for a field is an identifier", () => {
			...
			return expect(
				...
				.to.be.eventually.rejectedWith(
					'"name" with value "1notAnIdentifier" fails to match the required pattern: ' +
						"/^[a-zA-Z_][a-zA-Z0-9_]*$/",
				)
				.and.to.have.property("statusCode", 400);
		});
		...
		it("should require type id for a field", () => {
			...
			return expect(
				...
				.to.be.eventually.rejectedWith('"typeId" is required')
				.and.to.have.property("statusCode", 400);
		});
		...
		it("should not allow unsupported type ids", () => {
			...
			return expect(
				...
				.to.be.eventually.rejectedWith(
					'"typeId" must be one of [Path, String, Integer, Boolean, Single, Double]',
				)
				.and.to.have.property("statusCode", 400);
		});
		...
		it("should require an include criteria", () => {
			...
			return expect(
				...
				.to.be.eventually.rejectedWith('"include" must contain at least 1 items')
				.and.to.have.property("statusCode", 400);
		});
		...
		it("should require a schema in an include criteria", () => {
			...
			return expect(
				...
				.to.be.eventually.rejectedWith('"schema" is not allowed to be empty')
				.and.to.have.property("statusCode", 400);
		});
	});
	...
	describe("when ingesting change sets with string OTs and arrays", () => {
		...
		before(() => {
			...
			indexName = "text";
			indexDef = {
				fields: [{ typeId: "String", name: "text" }],
				include: [{ schema: "NodeProperty" }],
			};
			const meta = {
				materializedHistory: {
					enabled: true,
	...
	describe("when branching", () => {
		...
		it("should be possible to get the index MV of both the original and the new branch", async () => {
			...
			expect(firstBranchResult).to.eql(
				createSingleColumnIndexMV([
			...
			expect(secondBranchResult).to.eql(
				createSingleColumnIndexMV([
					[firstSample, "property500"],
