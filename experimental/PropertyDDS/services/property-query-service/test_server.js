...
let jsonOptions = (yargs) => {
	yargs
		.describe("files", "JSON files to load")
		.default("files", [
			"branchInfo_multipleCommits2.json",
			"branchInfo_references.json",
			"branchInfo_index.json",
		]);
};
...
let argv = require("yargs")
	...
	.command(
		["json [files..]", "$0 [files..]"],
		"Initialize the branches from json files",
		jsonOptions,
	)
...
require("./server.js").then(async (materializedHistoryServer) => {
	...
	var addCommits = async function (
		in_branchGuid,
		in_parentGuid,
		in_nextCommitFunction,
		in_commitCounter,
	) {
	...
	let createBranchWithCommits = function ({
		branchGuid,
		rootCommitGuid,
		metaData,
		commitFunction,
	...
	if (command === "synthetic" || command === "all") {
		...
		while (payloadSize < desiredPayLoadSize) {
			...
			if (desiredPayLoadSize - payloadSize >= 100) {
				...
				str = _.range(desiredPayLoadSize - payloadSize - 16)
					.map(() => "x")
					.join("");
			}
		...
		await createBranchWithCommits({
			...
			let leafs =
				await materializedHistoryServer._materializedHistoryService._getAllLeafsForCommit({
					guid: commitGUIDs[commitGUIDs.length - 1],
				});

			console.log("Leafs in final commit: " + leafs.length);
			let totalFinalSize = 0;
			leafs.forEach((x) => {
				totalFinalSize += JSON.stringify(x.changeSet).length;
			});
			console.log("Size of final commit CS: " + Math.floor(totalFinalSize / 1024));
			console.log("Average leaf size: " + Math.floor(totalFinalSize / 1024 / leafs.length));
		});
