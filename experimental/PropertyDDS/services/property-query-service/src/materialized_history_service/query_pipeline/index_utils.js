...
class IndexUtils {
	...
	static getIndexTraversalParamsFromFilter(params) {
		...
		if (params.typeId) {
			...
			const matchingCriteria = params.indexDef.include.find(
				(includeCriteria) => includeCriteria.schema === typeId,
			);
			if (!matchingCriteria) {
				throw new OperationError(
					`Cannot query index '${params.indexName}' on branch '${params.branchGuid}'. Index` +
						` does not cover schema '${typeId}'`,
					"GetIndexMV",
			...
			throw new OperationError(
				"Querying an index currently requires a typeId",
				"GetIndexMV",
				HTTPStatus.BAD_REQUEST,
				OperationError.FLAGS.QUIET,
			);
		...
		if (params.where) {
			const keys = Object.keys(params.where).filter(
				(key) => !SUPPORTED_WHERE_CONDITIONS.includes(key),
			);
			if (keys.length > 0) {
				const conditionsText = `'${SUPPORTED_WHERE_CONDITIONS.join("', '")}'`;
				throw new OperationError(
					`Only conditions ${conditionsText} are currently supported for indices`,
					"GetIndexMV",
					HTTPStatus.BAD_REQUEST,
					OperationError.FLAGS.QUIET,
				);
			...
			const getValueFromConditions = (conditions) => {
				...
				for (const propertyName of Object.keys(conditions)) {
					const index = params.indexDef.fields.findIndex(
						(field) => field.name === propertyName,
					);
					if (index === -1) {
						throw new OperationError(
							`Cannot query index '${params.indexName}' on branch '${params.branchGuid}'. ` +
								`Index does not include field '${propertyName}'`,
							"GetIndexMV",
							HTTPStatus.BAD_REQUEST,
							OperationError.FLAGS.QUIET,
						);
				...
				for (let i = 0; i < value.length; i++) {
					if (value[i] === NOT_PROVIDED) {
						throw new OperationError(
							"Index scan is not currently supported. A value for field " +
								`'${params.indexDef.fields[i].name}' was not provided.`,
							"GetIndexMV",
							HTTPStatus.BAD_REQUEST,
							OperationError.FLAGS.QUIET,
						);
			...
			if (params.where.lte) {
				...
				valueRanges.push([
					_.times(params.indexDef.fields.length, _.constant(undefined)),
					value,
				]);
				values.push(value);
			}
	 ...
	 */
	static extractPathsFromIndexMV(
		changeSet,
		fieldCount,
		pathPrefix = "",
		depthLimit = Infinity,
		...
		const getSortedKeys = (cs, level) => {
			...
			keys = _.orderBy(
				keys,
				(key) => order.sortKeyEncoder(key, level),
