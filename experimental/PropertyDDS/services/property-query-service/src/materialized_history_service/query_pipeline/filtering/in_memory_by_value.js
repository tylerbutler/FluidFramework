...
class InMemoryByValue {
	...
	static async filterByValue(from, changeSet, limitedPaths) {
		...
		let retainedPaths = await InMemoryByValue._filterPaths(
			tokenizedPagingPath,
			from,
			changeSet,
		);
	...
	static async _filterPaths(tokenizedPagingPath, from, changeSet) {
		...
		const _getPath = _.memoize(
			(stack, length) =>
				stack.slice(0, length).map(PathHelper.quotePathSegmentIfNeeded).join("."),
						...
						if (keyForContext) {
							...
							paths[
								_getPath(
									...
									-1 * tokenizedFieldsToGather[keyForContext].length,
								)
							][keyForContext] = {
	...
	static _validatePrimitiveType(type) {
		if (!TypeIdHelper.isPrimitiveType(type)) {
			throw new OperationError(
				`Attempting to perform filtering on a non-primitive field, type was ${type}`,
	...
	static _evaluateMatch(expected, actual, typeId) {
		...
		if (typeId !== "String") {
			throw new OperationError(
				`Attempting to perform regex match on non-string field, type was ${typeId}`,
				"_evaluateMatch",
				HTTPStatus.BAD_REQUEST,
				OperationError.FLAGS.QUIET,
			);
