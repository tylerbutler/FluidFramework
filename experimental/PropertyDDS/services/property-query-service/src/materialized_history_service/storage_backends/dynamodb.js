...
const logger = ModuleLogger.getLogger(
	"HFDM.MaterializedHistoryService.StorageBackends.DynamoDBBackend",
);
...
class DynamoDBBackend {
	...
	constructor(in_params) {
		...
		this._oldS3DeleteCargo = asyncCargo(
			this._deleteS3Items.bind(this),
			this._maxBatchDeleteSize,
		);
		this._uploadToS3Queue = asyncQueue(
			this._uploadS3Item.bind(this),
			this._maxConcurrentS3Uploads,
		);
	...
	async _writeBatch(tasks) {
		...
		tasks.forEach((t) => {
			let s3Key =
				t.value.length > this._largeNodeThreshold ? `${t.key}:rev:${generateGUID()}` : null;
		...
		let recordsWithPreviousS3 = tasks.filter(
			(t) =>
				t.extra &&
	...
	async _fetchBatch(consistent, tasks) {
		...
		getParams[this._tableName] = _.uniqWith(
			tasks.map((r) => r.key),
			(k1, k2) => k1.PK === k2.PK,
		);
	...
	store(in_batch, in_nodeRef, in_value) {
		if (!this._writeCargos[in_batch.guid]) {
			throw new OperationError(
				"Trying to store in a non-started batch",
				"CreateCommit",
				HTTPStatus.INTERNAL_SERVER_ERROR,
			);
		}
	...
	update(in_batch, in_nodeRef, in_value, in_extra) {
		if (!this._writeCargos[in_batch.guid]) {
			throw new OperationError(
				"Trying to update in a non-started batch",
				"CreateCommit",
				HTTPStatus.INTERNAL_SERVER_ERROR,
			);
		}
		...
		if (this._batchStatuses[in_batch.guid] === BATCH_STATUSES.FINISHED) {
			throw new OperationError(
				"Trying to update in an already finished batch",
				"CreateCommit",
				HTTPStatus.INTERNAL_SERVER_ERROR,
			);
		}
	...
	_get(in_nodeRef, in_consistent) {
		...
		return new Promise((resolve, reject) => {
			...
			let queue = in_consistent
				? this._nodeFetchConsistentCargoQueue
				: this._nodeFetchInconsistentCargoQueue;
				...
				handler: {
					resolve: async (result) => {
						if (!result) {
							...
							if (result.s3Key) {
								logger.debug(
									"Done reading node.  Beginning to read from S3",
									primaryKey,
									result.s3Key,
								);
								try {
							...
							AsyncContext.incrementInContext(
								"rcuUsed",
								Math.ceil(result.value.length / readCostRatio),
							);
