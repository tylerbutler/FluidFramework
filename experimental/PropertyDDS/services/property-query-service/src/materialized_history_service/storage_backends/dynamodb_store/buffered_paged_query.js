...
class BufferedPagedQuery extends PagedQuery {
	...
	pause() {
		if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
			this._log(
				`pause(${this._params.queryName}), page: ${this._pageIndex}` +
	...
	resume() {
		if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
			this._log(
				`resume(${this._params.queryName}), page: ${this._pageIndex}` +
					`, paused: ${this._rowQueue.paused}`,
			);
		}
...
async function _bufferRow(row, rowIndex) {
	if (!this._isCancelled) {
		if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
			this._log(
				`_bufferRow(${this._params.queryName}), page: ${this._pageIndex}` +
...
function _onBufferDrained() {
	if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
		...
		if (this._rowHandlerError) {
			msg +=
				", error: " +
				(this._rowHandlerError.message
					? this._rowHandlerError.message
					: this._rowHandlerError);
		}
		this._log(msg);
	}
