...
async function _onError(error) {
	...
	const errorFilterAbort =
		typeof this._config.errorFilter === "function" && this._config.errorFilter(error) === false;
	...
	let jitteredTimeoutMilliSec =
		this._config.jitter === undefined
	...
	if (this._config.taskTimeoutMilliSec && this._config.taskTimeoutMilliSec > 0) {
		...
		if (elapsedAtRetryTime >= this._config.taskTimeoutMilliSec) {
			...
			if (logger.isLevelEnabled(ModuleLogger.levels.DEBUG)) {
				logger.debug(
					`${msgPrefix}next retry in ` +
						`${jitteredTimeoutMilliSec.toFixed(
							0,
						)} ms would cause the total duration to exceed ` +
	...
	diags.taskName = this._taskName;
	logger.info(
		`${msgPrefix}Transient failure. Retry in ` + `${jitteredTimeoutMilliSec.toFixed(0)} ms.`,
		diags,
	);
...
function _setNextRetryTimeout() {
	...
	if (_isMaxTimeoutConfigured.call(this)) {
		this._currentRetryTimeout = Math.min(
			this._currentRetryTimeout,
			this._config.maxSingleTimeoutMilliSec,
		);
	}
}
