...
class DynamoDBClient {
	...
	async _ddbApiCall(fnName) {
		...
		if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
			...
			logger.trace(
				`${fnName}${
					tableName ? ":" + tableName : ""
				} ${Logging.getDbInStyle()} ${JsonUtils.stringify(filteredLog, null, 2)}`,
			); // eslint-disable-line
		}
		...
		if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
			let filteredLog = Logging.filterLogObject(
				timedResult.result,
				Logging.filterKeys.output,
			);
			filteredLog.opId = opId;
			...
			logger.trace(
				`${fnName}${
					tableName ? ":" + tableName : ""
				} ${Logging.getDbOutStyle()} ${JsonUtils.stringify(filteredLog, null, 2)}`,
			); // eslint-disable-line
		}
	...
	async batchGetItem(batchParams, options = { consistentRead: false }) {
		options.consistentRead = _.isUndefined(options.consistentRead)
			? false
		...
		const taskCb = async (retryCount) => {
			try {
				...
				_.mapValues(result.Responses, (marshalledRows, tableName) => {
					rowsPerTable[tableName] = rowsPerTable[tableName]
	...
	async connect() {
		...
		try {
			...
			logger.error(
				`Failed to describe table: ${tableName}: ${error.message ? error.message : error}`,
			);
			throw error;
		}
	...
	async getItem(tableName, pk, options) {
		...
		if (!result.Item && !options.consistentRead) {
			logger.info(
				`Retrying getItem ${JSON.stringify(pk)} on table ${tableName} with consistent read`,
			);
	...
	setQueryParams(tableName, pk, params) {
		params.ddbParams = _.extend(
			{
				TableName: tableName,
				ReturnConsumedCapacity: this._config.returnConsumedCapacity,
				ConsistentRead: _.isUndefined(params.consistentRead)
					? false
		...
		params.ddbParams.ExpressionAttributeValues =
			AWS.DynamoDB.Converter.marshall(conditionValueMap);
	...
	async updateItem(tableName, pk, updates, condition, removeArray) {
		const ddbParams = this._paramFactory.createUpdateItem(
			tableName,
			pk,
			updates,
			condition,
	...
	async waitForIndex(
		...
		indexName,
		action,
		maxWaitTimeMilliSec = Number.MAX_SAFE_INTEGER,
	) {
		const operation = "describeTable";
		...
		while (chrono.stop().elapsedMilliSec() < maxWaitTimeMilliSec) {
			...
			if (action === "delete") {
				if (
					...
					data.Table.GlobalSecondaryIndexes.length === 0
				) {
					return;
				}

				const gsiEntry = _.find(
					data.Table.GlobalSecondaryIndexes,
	...
	async waitForTable(tableName, maxWaitTimeMilliSec) {
		...
		throw new OperationError(
			`Timeout waiting for table to become ready: ${tableName}. ` +
				`Elapsed: ${chrono.elapsedMilliSec().toFixed(0)} ms`,
			operation,
			HttpStatus.SERVICE_UNAVAILABLE,
		);
	...
	await PluginManager.instance.systemMonitor.startSegment(segmentName, true, async () => {
		...
		try {
			...
			if (elapsedMilliSec >= this._config.elapsedWarningThresholdMilliSec) {
				...
				const requestId =
					result && result["$response"] ? result["$response"].requestId : undefined;
				if (requestId) {
					const diags = {
						operation: fnName,
