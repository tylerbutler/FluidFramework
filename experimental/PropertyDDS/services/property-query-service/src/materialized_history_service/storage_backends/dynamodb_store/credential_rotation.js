...
class CredentialRotation {
	...
	constructor() {
		...
		if (
			ddbSettings &&
			ddbSettings.awsRole &&
			ddbSettings.awsRole.arn &&
			ddbSettings.awsRole.prefix
		) {
	...
	async _onRotation(error, creds) {
		...
		try {
			...
			logger.info(
				`Successfully connected with renewed credentials using ${JSON.stringify(
					this._role,
				)}. ` + JSON.stringify(_.pick(ddbCreds, "endpoint", "region")),
			);
		} catch (connectError) {
			logger.error(
				`Failed to connect with renewed credentials using ${JSON.stringify(this._role)}:`,
				connectError,
			);
