...
class PagedQuery {
	...
	async all(concurrencyLimit = 1, rcuTokenBucket) {
		...
		if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
			this._log(
				`all(${this._params.queryName}), concurrencyLimit ` +
	...
	cancel() {
		if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
			this._log(
				`cancel(${this._params.queryName}), page: ${this._pageIndex}. Paging canceled.`,
			);
		}
	...
	hasNext() {
		const hasNext =
			!this._isCancelled &&
			(this._pageIndex === 0 || !!this._params.ddbParams.ExclusiveStartKey);
		...
		return [
			{
				pagination: {
					...
					next: queryResult.LastEvaluatedKey
						? AWS.DynamoDB.Converter.unmarshall(queryResult.LastEvaluatedKey)
						: undefined,
				},
				rows,
			},
	...
	async _cancelableRowCb(row, rowIndex) {
		if (this._isCancelled) {
			if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
				this._log(
					`_cancelableRowCb(${this._params.queryName}), page ${this._pageIndex}` +
