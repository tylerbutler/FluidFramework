...
(function () {
	...
	var FileStore = function (params) {
		...
		if (!this._path) {
			...
			this._logger.warn(
				"FileStore is not configured. Big change sets will be stored in: '" +
					homeDir +
					"'. You may override this location with the 'fileStore:config:path' setting.",
			);
			this._path = homeDir;
		}
	...
	var _deleteDirIfEmpty = function (dirName) {
		...
		return promise.then(function (isEmpty) {
	...
	FileStore.prototype.exists = function (key) {
		...
		return promise
			...
			.then((result) => true)
			.catch((error) =>
				error && error.code === ERROR_CODES.ENOENT.code ? false : Promise.reject(error),
			);
	};
	...
	FileStore.prototype.getObject = function (key) {
		...
		return promise.then(_logResult.bind(this, opName)).catch((error) => {
