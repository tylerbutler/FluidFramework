	...
	async getObject(key) {
		...
		return Promise.mapSeries(this._regionalS3s, async (regionalStore) => {
			if (!getObjectResult.result) {
				try {
					getObjectResult.result = await this._regionalCall(regionalStore, "getObject", [
						key,
					]);
				} catch (error) {
	...
	async putObject(key, buffer, tags) {
		...
		if (firstError) {
			if (successfulRegions.length > 0) {
				try {
					await Promise.all(
						_.map(successfulRegions, (regionalStore) => rollbackPromise(regionalStore)),
					);
				} catch (rollbackError) {
					// intentionally ignored
				}
		...
		return Promise.mapSeries(this._regionalS3s, async (regionalStore) => {
			if (!getObjectUrlResult.result) {
				try {
					...
					if (exists) {
						getObjectUrlResult.result = await this._regionalCall(
							regionalStore,
							"getObjectUrl",
							[key, expiry],
						);
