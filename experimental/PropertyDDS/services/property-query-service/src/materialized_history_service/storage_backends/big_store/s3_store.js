...
(function () {
	...
	var S3Store = function (params) {
		...
		this._timingLogger = ModuleLogger.getLogger(
			"HFDM.PropertyGraphStore.BigStore.S3Store.Timing",
		);

		this._params.options.acl = this._params.options.acl || "bucket-owner-full-control";

		if (
			this._params.options.signedUrlExpirySec &&
			this._params.options.signedUrlExpirySec <= 0
		) {
			throw new RangeError(
				`signedUrlExpirySec must be greater than 0, is ${this._params.options.signedUrlExpirySec}`,
			);
		}
	...
	S3Store.prototype.getObjectUrl = async function (key, expiry) {
		...
		_logQuery.call(this, "getSignedUrl", {
			Method: methodName,
			Params: params,
		});
		...
		_logResult.call(this, "getSignedUrl", {
			Method: methodName,
			Url: "redacted",
		});
