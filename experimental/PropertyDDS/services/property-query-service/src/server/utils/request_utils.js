...
(function () {
	...
	RequestUtils.requestWithRetries = function (in_params, in_callback) {
		...
		in_params.requestCallbackHandler =
			in_params.requestCallbackHandler || RequestUtils.handleRequestCallback;
		...
		const defaultErrorFilter = function (err) {
			...
			if (
				in_params.requestParams.forever === true &&
				cancelKeepAliveErrors.indexOf(err.code) > -1
			) {
				if (!regexp || !(err.message || "").match(regexp)) {
					logger.warn(
						`${in_params.logMessage} ${err.message}, without keepAlive, ${tracking}`,
					);
				}
			...
			if (
				Array.isArray(in_params.retryErrorStrings) &&
				_.find(in_params.retryErrorStrings, function (item) {
			...
			// Retry options
			{
				...
				interval: function (retryCount) {
					...
					if (in_params.useExponentialBackoff) {
						...
						const tentativeInterval =
							Math.random() * in_params.initialRetryInterval * 0.25 +
						...
						return Math.floor(
							Math.random() * in_params.initialRetryInterval * 0.25 +
								in_params.initialRetryInterval,
						);
					}
				},
				...
				if (tracking) {
					...
					if (err) {
						...
						if (400 <= err.statusCode && err.statusCode < 500) {
							logger.warn(
								`${uri} (${duration}): code: ${err.statusCode}, failed, ${tracking}`,
							);
						} else {
							logger.error(
								`${uri} (${duration}): code: ${err.statusCode}, err: ${err.message}, failed, ${tracking}`,
							);
						}
					} else {
