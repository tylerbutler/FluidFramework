...
class Rotation {
	...
	addCallback(onRotation) {
		if (typeof onRotation !== "function") {
			throw new Error(
				"Failed to execute `addCallback`: The provided callback is not a function.",
			);
		}
	...
	removeCallback(onRotation) {
		if (typeof onRotation !== "function") {
			throw new Error(
				"Failed to execute `removeCallback`: The provided callback is not a function.",
			);
		}
	...
	async _startRotation() {
		...
		try {
			...
			timeoutMilliSec = Math.max(
				data.Credentials.Expiration.getTime() -
					new Date().getTime() -
					this._params.safeMarginMs,
			...
			logger.info(
				`Credentials for ${JSON.stringify(this._role)} will rotate on: ` +
			...
			logger.error(
				`Failed to obtain temporary credentials for ${JSON.stringify(this._role)}:`,
				error,
			);
		}
...
const credsRotation = {
	...
	addRotation(role, onRotation, params) {
		if (typeof onRotation !== "function") {
			throw new Error(
				"Failed to execute `addRotation`: The provided callback is not a function.",
			);
		}
