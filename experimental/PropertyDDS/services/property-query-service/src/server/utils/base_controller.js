...
(function () {
	...
	BaseController.prototype.render = function (in_response, in_body, in_statusCode, in_options) {
		...
		in_response.writeHead(in_statusCode, {
		...
		if (
			in_statusCode === HttpStatus.BAD_REQUEST &&
			_.isEmpty(in_body.errors) &&
			!_.isError(in_body.errors)
		) {
			in_body = {
				errors: [
					{
						message: "Bad Request. One or more parameters are malformed or missing.",
					},
				],
			};
