...
(function () {
	...
	var BaseServer = function (in_params) {
		...
		this._healthEndpoints = _.compact(
			_.uniq(["/health", "/v1/health", "/v2/health", this._params.healthRoute]),
		);
		...
		this._shortServiceName =
			this._params.shortServiceName || process.env.SHORT_SERVICE_NAME || "BS";
		...
		this._pendingRequestTracker = new PendingRequestTracker(
			this._server,
			systemMonitor,
			in_params.softOngoingRequests,
		);
		this._diagsController = new DiagsController(
			this,
			this._params,
			this._pendingRequestTracker,
		);
			...
			this._app.use(function (req, res, next) {
				...
				if (that._logger && that._logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
					...
					if (_.indexOf(that._logHTTPRequests.skip, pathname) === -1) {
						...
						that._logger.trace(
							"HTTP >>>> [" + req.method + "] [" + req.url + "] [" + headers + "]",
						);
					}
				}
				next();
	...
	BaseServer.prototype.start = function (options) {
		...
		return that._instanceMonitor
			...
			.then(function () {
				...
				listen =
					that._params.listen &&
					(options && _.isBoolean(options.listen) ? options.listen : true);
				if (listen) {
					return that.listen();
				} else {
					return Promise.resolve();
	...
	var _initServer = function () {
		...
		if (this._params.initializeServer) {
			...
			this._app.get(["/isup"], function (req, res) {
				res.status(200).end();
			});

			if (!this._params || !this._params.basicAuth) {
				this._logger.info(
					"No basicAuth parameters defined, disabling /killswitch and /noop resources",
				);
				return;
			}
	...
	function _setupAdminEndpoints(app, basicAuth) {
		...
		if (!username || (!password && !passwordList)) {
			this._logger.info(
				"Missing basicAuth parameters, disabling /killswitch and /noop resources",
			);
			return;
		}
		...
		if (this._server.listening) {
			...
			const displayCount = () => {
				that._server.getConnections((err, count) => {
					if (err) {
						that._logger.error(
							`Failed getting active connection count(${this._name})`,
							err,
						);
					} else {
			...
			return new Promise((resolve, reject) => {
				...
				that._connectionManager.shutdown((err, result) => {
					...
					stoppingPromise
						.then(() => that._instanceMonitor.publish("stopped"))
						.then(() => {
							if (err) {
								that._logger.error(
									"Closing the server",
	...
	BaseServer.prototype.installSignalHandler = function () {
		...
		let sigHandler = function (signal) {
			...
			if (this._terminating > 1) {
				...
				const termTimeout =
					(this._params.termTimeout || settings.get("baseServer:termTimeout")) * 1000;
				setTimeout(() => {
