...
class RequestSignatureValidator {
	...
	validateSignature(branchGuidProvider) {
		return (req, res, next) => {
			...
			const theSigningKey = this._requestSigningKeys.find(
				(rsk) => new Date(rsk.expireAt) > parsedRequestTimestamp,
			).key;

			const computedRequestSignature = crypto
				.createHmac(requestAlgo, theSigningKey)
