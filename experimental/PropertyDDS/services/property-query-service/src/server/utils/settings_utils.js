...
module.exports._getUserHome = function () {
	return process.env.HOME || process.env.USERPROFILE || process.env.PWD || __dirname;
};
 ...
 */
module.exports._checkValueTypes = function (
	keys,
	isExpectedType,
	expectedType,
	configValues,
	...
	for (var keyItr = 0; keyItr < keys.length; keyItr++) {
		...
		if (!isExpectedType(settingVal)) {
			...
			try {
				...
				throw new Error(
					"Unable to parse value for " +
						...
						expectedType +
						". " +
						'The parsed string begins with "' +
						(settingVal && settingVal.slice(0, 5)) +
						'...". ' +
						"The error is " +
						err.message,
				);
			...
			if (!isExpectedType(parsedVal)) {
				throw new Error(
					"Incorrect type for " + settingKey + "." + "Expected " + expectedType + ".",
				);
			} else {
				newVal = parsedVal;
			}
