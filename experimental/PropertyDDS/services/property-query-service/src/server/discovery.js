		...
		if (process.env.CONTAINER_MAPPING) {
			try {
				...
				if (containerMapping.hostIp && containerMapping.portMapping[myContainerPort]) {
					return {
						host: containerMapping.hostIp,
						port: String(containerMapping.portMapping[myContainerPort]),
					};
				} else {
					let msg =
						"Failed to discover the external host:port mapping. " +
						process.env.CONTAINER_MAPPING;
					logger.error(msg);
					throw new Error(msg);
				}
			} catch (e) {
