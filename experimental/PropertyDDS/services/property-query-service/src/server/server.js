...
Server.prototype.init = async function () {
	...
	if (settings.get("enableRedisBranchAssignation") === true) {
		this._metrics = {
			load: {
				metricName: "MH_load",
				getValue: () => this._loadManager._calculateLoad(),
			},
		};
	} else {
		this._metrics = {
...
Server.prototype.term = async function () {
	...
	await Promise.all([
		settings.get("enableRedisBranchAssignation") === true
			? this._loadManager.tearDown()
			: Promise.resolve(),
...
Server.prototype._fetchHealth = function () {
	...
	this._healthResult = _.extend(this.getBasicHealth(), {
		status: this._healthResult === -1 ? "failed" : "passed",
		errors: [],
	});
	this._healthStatusCode = statusCode;
	this._healthTimeout = setTimeout(
		this._fetchHealth.bind(this),
		settings.get("mh:health_polling_interval"),
	);
...
Server.prototype.getHealth = function (req, res) {
	if (this._healthStatusCode === -1) {
		this._fetchHealth().then((healthResult) => {
