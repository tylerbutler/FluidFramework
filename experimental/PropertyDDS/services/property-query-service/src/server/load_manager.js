...
class LoadManager {
	...
	_resetLoadShedCooldown() {
		this._loadShedCooldown = Math.floor(
			Math.random() * this._loadShedding.cooldownFactor * this._loadShedding.windowSize,
		);
	}
	...
	async _updateLoad() {
		...
		try {
			...
			await Promise.all(
				_.map(inactive, (branchGuid) => {
					this._storageManager.clearCacheForBranch(branchGuid);
					return this._branchAssignations.removeMhInstanceForBranch(
						branchGuid,
						this._myHost,
					);
				}),
		...
		if (this._loadShedCooldown === 0) {
			const average =
				this._loadMeasures.reduce((acc, current) => acc + current, 0) /
			...
			if (average > this._loadShedding.cpuThreshold) {
				...
				try {
					...
					if (branchesToShed.length > 0) {
						logger.info(
							`Removing branches ['${branchesToShed.join(
								"', '",
							)}'] from assignations to shed load`,
						);
						await Promise.all(
							_.map(branchesToShed, (branchGuid) => {
								this._storageManager.clearCacheForBranch(branchGuid);
								return this._branchAssignations.removeMhInstanceForBranch(
									branchGuid,
									this._myHost,
						...
						logger.warn(
							"No branches to deassign but server continues under heavy load",
						);
					}
				} catch (error) {
