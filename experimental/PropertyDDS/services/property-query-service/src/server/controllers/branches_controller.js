...
class BranchesController extends BaseController {
	...
	constructor(params) {
		...
		this._app.post(
			...
			this._requestSignatureValidator.validateSignature(
				BranchGuidProvider.branchGuidFromBodyGuid,
			),
		...
		this._app.post(
			...
			bodyParser.json({ limit: "100mb" }),
			this._requestSignatureValidator.validateSignature(
				BranchGuidProvider.branchGuidFromBodyGuids,
			),
		 ...
		 */
		this._app.get(
			this.getV1Paths("/branchDeletion/:taskGuid").concat([
				"/mhs/v1/branchDeletion/:taskGuid",
			]),
			bodyParser.json({ limit: "100mb" }),
		 ...
		 */
		this._app.post(
			this.getV1Paths("/branchDeletion/:taskGuid/retry").concat([
				"/mhs/v1/branchDeletion/:taskGuid/retry",
			]),
			bodyParser.json({ limit: "100mb" }),
			this._checkKeys([], ["taskGuid"]).bind(this),
		...
		this._app.get(
			...
			this._requestSignatureValidator.validateSignature(
				BranchGuidProvider.branchGuidFromParams,
			),
		 ...
		 */
		this._app.post(
			this.getV1Paths("/branch/:branchGuid/commit").concat([
				"/mhs/v1/branch/:branchGuid/commit",
			]),
			bodyParser.json({ limit: "100mb" }),
			this._requestSignatureValidator.validateSignature(
				BranchGuidProvider.branchGuidFromParams,
			...
			this._checkKeys(
				["guid", "parentGuid", "branchGuid", "changeSet", "meta"],
				["branchGuid"],
			).bind(this),
		 ...
		 */
		this._app.post(
			this.getV1Paths("/branch/:branchGuid/commitTask").concat([
				"/mhs/v1/branch/:branchGuid/commitTask",
			]),
			bodyParser.json({ limit: "100mb" }),
			this._requestSignatureValidator.validateSignature(
				BranchGuidProvider.branchGuidFromParams,
		 ...
		 */
		this._app.get(
			this.getV1Paths("/branch/:branchGuid/commit/:commitGuid").concat([
				"/mhs/v1/branch/:branchGuid/commit/:commitGuid",
			]),
			this._requestSignatureValidator.validateSignature(
				BranchGuidProvider.branchGuidFromParams,
			),
		 ...
		 */
		this._app.get(
			this.getV1Paths("/branch/:branchGuid/commit/:commitGuid/materializedView").concat([
				"/mhs/v1/branch/:branchGuid/commit/:commitGuid/materializedView",
			]),
			this._requestSignatureValidator.validateSignature(
				BranchGuidProvider.branchGuidFromParams,
			),
		 ...
		 */
		this._app.get(
			this.getV1Paths("/branch/:branchGuid/commit/:commitGuid/changeSet").concat([
				"/mhs/v1/branch/:branchGuid/commit/:commitGuid/changeSet",
			]),
			this._requestSignatureValidator.validateSignature(
				BranchGuidProvider.branchGuidFromParams,
			),
	...
	async getCommitMV(req, res, next) {
		...
		try {
			...
			await this._materializedHistoryService.waitUntilCommitApplied(
			...
			await this._systemMonitor.startWebTransaction("GetCommitMV", async () => {
				...
				let response = _.extend(mvAtCommit, {
					rootCommitGuid: branchInfo.rootCommitGuid,
				});
		...
		if (req.query.rangeStart && req.query.rangeEnd) {
			...
			if (rangeStart.length !== rangeEnd.length) {
				throw new OperationError(
					"Number of rangeStart and rangeEnd parameters must match",
					"GetCommit",
					HTTPStatus.BAD_REQUEST,
					OperationError.FLAGS.QUIET,
				);
	...
	async createDeleteBranchTask(req, res, next) {
		...
		try {
			let existingTaskForBranches =
				await this._materializedHistoryService.getDeleteBranchTaskByBranches({
			...
			const [task, taskPromise] =
				await this._materializedHistoryService.createDeleteBranchTask({
					taskGuid: taskGuid,
					branchGuids: req.body.branchGuids,
	...
	async getDeleteBranchTask(req, res, next) {
		...
		try {
			...
			await new Promise((resolve, reject) => {
				this._requestSignatureValidator.validateSignature(
					BranchGuidProvider.fromArgument(task.branchGuids),
				)(req, res, (err) => {
					if (err) {
						replied = true;
						reject(err);
