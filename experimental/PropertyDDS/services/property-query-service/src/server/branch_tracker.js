...
class BranchTracker {
	...
	removeInactive(since) {
		...
		_.each(this._branchUsage, (branchUsage, branchGuid) => {
			...
			if (
				!(branchUsage[branchUsage.length - 1] >= since) &&
				!this._writeQueue.isProcessing(branchGuid)
			) {
	...
	getSheddable(since) {
		...
		let sheddable = _.pickBy(
			this._branchUsage,
			(bu, branchGuid) => !this._writeQueue.isProcessing(branchGuid),
		);
		for (const branchGuid of Object.keys(sheddable)) {
			const priority = sheddable[branchGuid].reduce(
				(acc, cur) => acc + Math.max(cur - since, 0),
				0,
			);
			branchPriority.splice(_.sortedIndexBy(branchPriority, priority, "priority"), 0, {
				branchGuid,
				priority,
			});
		}
