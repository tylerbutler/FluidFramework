...
(function () {
	...
	HfdmRedisClient.prototype.connect = function () {
		...
		if (this.redis && this.redis.status === "connect") {
			return Promise.reject(
				new Error("Can't connect, a connection is already established to Redis"),
			);
		}
		...
		if (that.redis.status === "connect") {
			// TODO: Magic constant here
	...
	HfdmRedisClient.prototype._handlePubSubMessage = function (channel, message) {
		if (this._pubSubSubscriptions[channel]) {
			if (logger.isLevelEnabled(ModuleLogger.levels.DEBUG)) {
				logger.debug(
					"Broadcasting message for channel: " +
						channel +
						" to " +
						this._pubSubSubscriptions[channel].length +
						" callbacks. Message contents was: " +
			...
			logger.debug(
				"Received message for non-subscribed channel: " +
					channel +
	...
	HfdmRedisClient.prototype._handlePubSubPMessage = function (pattern, channel, message) {
		if (this._pubSubSubscriptions[pattern]) {
			if (logger.isLevelEnabled(ModuleLogger.levels.DEBUG)) {
				logger.debug(
					"Broadcasting message for pattern: " +
						pattern +
						" to " +
						this._pubSubSubscriptions[pattern].length +
						" callbacks. Message contents was: " +
						JSON.stringify(message),
				);
			}
			...
			logger.debug(
				"Received message for non-subscribed pattern: " +
					pattern +
					". Message contents was: " +
					JSON.stringify(message),
			);
	...
	HfdmRedisClient.prototype._addSubscriptionCallback = function (channel, callbackKey, callback) {
		...
		this._pubSubSubscriptions[channel].push({
	...
	HfdmRedisClient.prototype.publish = function (channel, message) {
		const that = this;
		return _checkConnected.call(this).then(function () {
	...
	const _subscribe = function (verb, channel, callbackKey, callback) {
		const that = this;
		const mustSubscribe =
			!that._pubSubSubscriptions[channel] || that._pubSubSubscriptions[channel].length <= 0;
	...
	const _unsubscribe = function (verb, channel, callbackKey) {
		...
		if (!this._pubSubSubscriptions[channel] || this._pubSubSubscriptions[channel].length <= 0) {
			...
			return _checkConnected.call(this).then(() => {
				return verb(channel);
			});
		}
	...
	HfdmRedisClient.prototype.subscribe = function (channel, callbackKey, callback) {
		return _subscribe.call(
			this,
			this.redis.subscribe.bind(this.redis),
			channel,
			callbackKey,
			callback,
		);
	...
	HfdmRedisClient.prototype.psubscribe = function (pattern, callbackKey, callback) {
		return _subscribe.call(
			this,
			this.redis.psubscribe.bind(this.redis),
			pattern,
			callbackKey,
			callback,
		);
	...
	HfdmRedisClient.prototype.unsubscribe = function (channel, callbackKey) {
		return _unsubscribe.call(
			this,
			this.redis.unsubscribe.bind(this.redis),
			channel,
			callbackKey,
		);
	};
	...
	HfdmRedisClient.prototype.punsubscribe = function (pattern, callbackKey) {
		return _unsubscribe.call(
			this,
			this.redis.punsubscribe.bind(this.redis),
			pattern,
			callbackKey,
		);
	};
	...
	HfdmRedisClient.prototype.psubscribeAll = function (pattern, callbackKey, callback) {
		if (this._cluster) {
			return Promise.all(
				this.redis.nodes("master").map((ri) => {
					return _subscribeAll.call(
						this,
						ri.psubscribe.bind(ri),
						pattern,
						callbackKey,
						callback,
					);
	...
	HfdmRedisClient.prototype.punsubscribeAll = function (pattern, callbackKey) {
		if (this._cluster) {
			return Promise.all(
				this.redis.nodes("master").map((ri) => {
					return _unsubscribeAll.call(
						this,
						ri.punsubscribe.bind(ri),
						pattern,
						callbackKey,
					);
				}),
	...
	HfdmRedisClient.prototype.scan = function (pattern) {
		...
		let promises = redisInstances.map(
			(ri) =>
	...
	HfdmRedisClient.prototype.disconnect = function () {
		...
		if (!this.redis) {
			...
			promise = Promise.reject(
				new Error(`Can't disconnect. Current status: ${this.redis.status}`),
			);
