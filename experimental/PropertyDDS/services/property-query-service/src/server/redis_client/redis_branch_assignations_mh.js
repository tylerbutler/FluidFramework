...
const luaProcs = {
	getMhInstanceForBranch: fs.readFileSync(
		path.join(luaProcFolder, "get_mh_instance_for_branch.lua"),
	),
	...
	markMhInstanceShuttingDown: fs.readFileSync(
		path.join(luaProcFolder, "mark_mh_instance_shuttingdown.lua"),
	),
	removeMhInstanceForBranch: fs.readFileSync(
		path.join(luaProcFolder, "remove_mh_instance_for_branch.lua"),
	),
};
...
RedisBranchAssignationsMH.prototype.connect = function () {
	...
	return this.client.connect().then(() => {
		this.client.defineCommand("getMhInstanceForBranch", {
			numberOfKeys: 1,
			lua: luaProcs.getMhInstanceForBranch,
		});

		this.client.defineCommand("removeMhInstance", {
			numberOfKeys: 1,
			lua: luaProcs.removeMhInstance,
		});

		this.client.defineCommand("reportMhFailure", {
			numberOfKeys: 1,
			lua: luaProcs.reportMhFailure,
		});

		this.client.defineCommand("confirmMhEviction", {
			numberOfKeys: 1,
			lua: luaProcs.confirmMhEviction,
		});

		this.client.defineCommand("upsertMhInstance", {
			numberOfKeys: 1,
			lua: luaProcs.upsertMhInstance,
		});

		this.client.defineCommand("upsertMhUserInstance", {
			numberOfKeys: 1,
			lua: luaProcs.upsertMhUserInstance,
		});

		this.client.defineCommand("markMhInstanceShuttingDown", {
			numberOfKeys: 1,
			lua: luaProcs.markMhInstanceShuttingDown,
		});

		this.client.defineCommand("removeMhInstanceForBranch", {
			numberOfKeys: 1,
...
RedisBranchAssignationsMH.prototype.getMhInstanceForBranch = function (branchId) {
	...
	return this.client.hget(MH_BRANCH_ASSIGNATIONS_HMAP, branchId).then((result) => {
		if (result === null) {
			return this.client.getMhInstanceForBranch(SHARDING_PREFIX, branchId).then((mhId) => {
				if (!mhId) {
					throw new Error("No MH instance available. Try again later.");
				}
				logger.debug(
					`getMhInstanceForBranch returns branch ${branchId} -> ${mhId} from LUA script`,
				);
				return mhId;
			});
		}
 ...
 *   must retry.
 */
RedisBranchAssignationsMH.prototype.reportFailure = function (
	branchId,
	mhId,
	mhUserId,
	thisMinute,
	...
	thisMinute = thisMinute !== undefined ? thisMinute : nowDate - (nowDate % (1000 * 60));
	...
	return this.client.reportMhFailure(
		SHARDING_PREFIX,
		branchId || "",
		mhId,
		mhUserId,
