...
class PSSClient {
	...
	async getCommitRange(params) {
		...
		try {
			...
			if (
				this._enableEviction &&
				(error.statusCode === undefined || error.statusCode >= 500)
			) {
				this._branchAssignations
					.reportFailure(params.branchGuid, pssHost, this._brokerId)
					.catch((ex) =>
						logger.warn(`Failed to report failure for instance ${pssHost}`, ex),
					);
			}
			throw error;
		}
	...
	async getBranch(branchGuid) {
		...
		try {
			...
			if (
				...
				this._branchAssignations
					.reportFailure(branchGuid, pssHost, this._brokerId)
					.catch((ex) =>
						logger.warn(`Failed to report failure for instance ${pssHost}`, ex),
					);
			}
	...
	async _getPSSInstance(branchGuid) {
		...
		if (this._isInstanceUnavailable(pss)) {
			throw new OperationError(
				"The PSS instance for this branch is temporarily unavailable, please try again shortly",
				"GetCommit",
