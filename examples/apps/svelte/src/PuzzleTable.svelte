...
const onRemoteCellChange = (coord: LatestValueClientData<CellCoordinate>) => {
	...
	if (grid[coord.value[0]][coord.value[1]] !== undefined) {
		console.debug("before", grid[coord.value[0]][coord.value[1]].owner);
		grid[coord.value[0]][coord.value[1]].owner = coord.client.sessionId;
		console.debug("after", grid[coord.value[0]][coord.value[1]].owner);
	}
};
