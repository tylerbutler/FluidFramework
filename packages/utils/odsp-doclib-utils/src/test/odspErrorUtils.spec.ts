...
describe("OdspErrorUtils", () => {
	...
	describe("createOdspNetworkError", () => {
		...
		it("ThrottlingError Test", () => {
			...
			assert.equal(
				networkError.retryAfterSeconds,
				100,
				"retryAfterSeconds should be preserved",
			);
		});
	});
	...
	describe("enrichError", () => {
		it("enriched with online flag", () => {
			...
			}) as GenericNetworkError & OdspError;
		...
		it("enriched with facetCodes", () => {
			const responseText =
				'{ "error": { "message":"hello", "code": "foo", "innerError": { "code": "bar" } } }';
		...
		it("error response with redirect location", () => {
			const responseText =
				'{ "error": { "message":"hello", "@error.redirectLocation":"url" } }';
			const error = createOdspNetworkError(
				"Test Message",
				404,
				undefined /* retryAfterSeconds */,
